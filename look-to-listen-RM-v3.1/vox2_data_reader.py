...
class DataReader(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            mixed_raw = raw_data_s1 + raw_data_s2
            norm = np.max(np.abs(mixed_raw)) * 1.1
            w1, w2, mixed = raw_data_s1 / norm, raw_data_s2 / norm, mixed_raw / norm

            target_mag_s1, _ = self.audio.wav2spec(w1)
            target_mag_s2, _ = self.audio.wav2spec(w2)
            mag_mix, phase_mix = self.audio.wav2spec(mixed)

            sample = {
                'audio_s1': np.asarray([target_mag_s1]),
                'audio_s2': np.asarray([target_mag_s2]),
                'audio_mix': np.asarray([mag_mix]),
                'phase_mix': np.asarray([phase_mix])
            }
            return sample
