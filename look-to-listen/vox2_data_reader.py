    ...
    def __getitem__(self, idx):
        try:
            ...
            sample = {
                'frames_s1': frame_s1,
                'frames_s2': frame_s2,
                'audio_s1': np.asarray([real_s1, imag_s2]),
                'audio_s2': np.asarray([real_s2, imag_s2]),
                'audio_mix': np.asarray([np.real(Zxx), np.imag(Zxx)]),
            }
            return sample
...
if __name__ == "__main__":
    ...
    db_loader = DataLoader(db, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, worker_init_fn=worker_init_fn)
