...
class fuseNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.biLSTMLayer = nn.LSTM(8 * 321, hidden_size=200, num_layers=1, bidirectional=True, batch_first=True)
        self.fc3 = nn.Sequential(
            nn.Linear(400, 600), nn.ReLU(True), nn.Linear(600, 600), nn.ReLU(True), nn.Linear(600, 600), nn.ReLU(True), nn.Linear(600, 321 * 2 ),
            )
    ...
    def forward(self, fuse_feature):
        ...
        out = torch.reshape(out, [-1, 297, 321, 1, 2])    # [batch , 297, 321 ,2]
...
if __name__ == "__main__":
    ...
    test_audio_input = torch.randn(3, 1, 297, 321).to(device_0)
